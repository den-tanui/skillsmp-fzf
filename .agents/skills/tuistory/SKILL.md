...
description: |
  ...
  **Important:** tuistory requires Bun runtime. The installed `tuistory` CLI may point to Node.js which will fail. Use the `~/bin/tuistory` wrapper (created automatically) or run directly with `bun /usr/lib/node_modules/tuistory/dist/cli.js`.
---
...
tuistory requires **Bun** runtime. Verify installation:
```bash
which bun && bun --version
```

If `tuistory` command fails with "Failed to connect to relay", the symlink may be pointing to Node.js. Fix by using the Bun wrapper:

```bash
# Create wrapper in ~/bin (already in PATH via .zshrc)
cat > ~/bin/tuistory << 'EOF'
#!/bin/bash
exec /usr/bin/bun /usr/lib/node_modules/tuistory/dist/cli.js "$@"
EOF
chmod +x ~/bin/tuistory
